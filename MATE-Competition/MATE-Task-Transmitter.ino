...
void encoder_test() {
  ...
  Serial.println("Target depth confirmed automatically by sensor.");
  Serial.println("--------------------------------------------\n");
  ...
  Serial.println(  "|           (Tolerance: +/-0.20m)            |");
  Serial.println(  "|--------------------------------------------|");

  while (count_0_4m < 0) {

    Serial.println("Press any key to extend one step, 'r' to retract, 'x' to abort.");
    ...
    // Sensor confirmation — larger +/-0.20m 
    if (depth >= (0.4f - ENCODER_TOLERANCE) &&
        depth <= (0.4f + ENCODER_TOLERANCE)) {
      ...
      Serial.println("Encoders for 0.4m locked. Proceeding to 2.5 m.\n");
    }
  }
  ...
  while (count_2_5m < 0) {
    ...
    bool ok = run_step(cmd != "r", depth, pressure);
    if (!ok) return; // limit switch abort

    // Sensor confirmation — +/-0.20m
    if (depth >= (2.5f - ENCODER_TOLERANCE) &&
        depth <= (2.5f + ENCODER_TOLERANCE)) {
      count_2_5m = piston_position;
      Serial.println("\n 2.5m TARGET CONFIRMED BY SENSOR");
      Serial.print("Encoder count: "); Serial.println(count_2_5m);
      Serial.print("Sensor depth: "); Serial.print(depth, 3);   Serial.println(" m");
      Serial.print("Pressure: "); Serial.print(pressure, 2); Serial.println(" kPa");
      Serial.println("Encoders for 2.5m locked. Returning to surface.\n");
    }
  }
  Serial.println("\n|--------------------------------------------|");
  ...
  Serial.println(count_0_4m >= 0 ? String(count_0_4m) : "NOT REACHED");
  Serial.print("  #define ENCODER_COUNT_2_5M    ");
  Serial.println(count_2_5m >= 0 ? String(count_2_5m) : "NOT REACHED");
  Serial.println("\nEncoder calibration complete.");
}

// Helper method for each step: extending piston and confirming depths using sensor readings
bool run_step(bool extend, float &depth, float &pressure) {
  if(extend) { 
    piston_out(); 
  ...
  // Check sensor twice with delay to see if readings differ by more than 0.01m
  // If unstable, keep looping
  do {
  ...
  // Save stable values and update final encoder count depth
  depth = reading_b;
  pressure = p;
  Serial.println();
